...
export class PedidosService {
  private pedidosSubject = new BehaviorSubject<Pedido[]>(this.carregarStorage('pedidos_v4'));
  private historicoSubject = new BehaviorSubject<Pedido[]>(this.carregarStorage('historico_vendas'));
  ...
  private salvarStorage(): void {
    localStorage.setItem('pedidos_v4', JSON.stringify(this.pedidosSubject.value));
    localStorage.setItem('historico_vendas', JSON.stringify(this.historicoSubject.value));
  }

  adicionarPedido(pedido: Pedido): void {
    pedido.status = 'pendente';
    pedido.criadoEm = new Date().toISOString(); // ← adiciona
  ...
  adicionarComprovante(numPedido: number, imagem: string): void {
    const pedidos = this.pedidosSubject.value.map(p =>
      p.numPedido == numPedido ? { ...p, status: 'validacao_pendente', comprovante: imagem } : p
    );
    this.pedidosSubject.next(pedidos);
    this.salvarStorage();
  }
  ...
  removerPedido(numPedido: number): void {
    this.pedidosSubject.next(this.pedidosSubject.value.filter(p => p.numPedido != numPedido));
    this.salvarStorage();
  }

  finalizarPedido(numPedido: number): void {
    const pedido = this.pedidosSubject.value.find(p => p.numPedido == numPedido);
    if (pedido) {
